...
app.get('/public/home', async (c) => {
  ...
  const tagId = c.req.query('tagId');     // 具体某个标签ID筛选
  const catId = c.req.query('catId');     // 分类ID筛选
  ...
  sql += ` ORDER BY r.id DESC LIMIT 100`; // 限制数量防止过大
...
// 6. 【新】获取标签墙 (用于展示 艺人/番组 图片列表)
app.get('/public/tags', async (c) => {
    const type = c.req.query('type'); // '番组' 或 '艺人'
    if (!type) return c.json([]);
    const res = await c.env.DB.prepare('SELECT * FROM tags WHERE type = ? ORDER BY id DESC').bind(type).all();
    return c.json(res.results);
});

// 7. 辅助: 查找标签图片 (用于管理员自动填入)
...
app.post('/admin/users/batch', async (c) => {
    ...
    const { userIds, action } = await c.req.json(); const ph = userIds.map(()=>'?').join(',');
    ...
    else if(action==='ban') { const users = await c.env.DB.prepare(`SELECT email FROM users WHERE id IN (${ph})`).bind(...userIds).all(); for(const us of users.results) await c.env.DB.prepare('INSERT OR IGNORE INTO blacklist (email, reason) VALUES (?, "批量拉黑")').bind(us.email).run(); await c.env.DB.prepare(`DELETE FROM users WHERE id IN (${ph})`).bind(...userIds).run(); }
    return c.json({success:true});
});
...
app.get('/admin/resources', async (c) => { const t=c.req.header('Authorization')?.split(' ')[1]; const u=await verifyToken(t,c.env.JWT_SECRET); if(!u||u.role!=='admin') return c.json({error:'无权'},403); const q=c.req.query('q'); const catId=c.req.query('catId'); let sql='SELECT * FROM resources'; let params=[]; let conds=[]; if(q){conds.push("title LIKE ?");params.push(`%${q}%`);} if(catId){conds.push("category_id = ?");params.push(catId);} if(conds.length>0) sql+=' WHERE '+conds.join(' AND '); sql+=' ORDER BY id DESC LIMIT 50'; const r=await c.env.DB.prepare(sql).bind(...params).all(); return c.json(r.results); });
app.post('/admin/resource/delete', async (c) => { const t=c.req.header('Authorization')?.split(' ')[1]; const u=await verifyToken(
