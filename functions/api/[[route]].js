...
async function syncUserQuota(env, user, todayStr) {
  ...
  if (user.last_unlock_date === yesterday) {
      ...
      let maxCap = 3;
      if (newConsecutive > 20) maxCap = 5;
      else if (newConsecutive > 10) maxCap = 4;
      newLimit = Math.min((user.daily_limit || 1) + 1, maxCap);
  } else if (user.last_unlock_date === todayStr) {
      // 理论上不会进这里，因为上面 check 了 calc_date
      return user; 
  } else {
      newConsecutive = 0; 
      newLimit = 1;       
...
app.post('/auth/send-code', async (c) => {
  ...
  const user = await c.env.DB.prepare('SELECT id FROM users WHERE email = ?').bind(email).first(); // 优化：只查 id
...
app.post('/auth/login', async (c) => {
  ...
  if (isAdmin) { 
      if (loginId === c.env.ADMIN_USER && password === c.env.ADMIN_PASSWD) {
          ...
          await c.env.DB.prepare('INSERT INTO admin_logs (ip, ua) VALUES (?, ?)').bind(ip, ua).run();
          // 优化：异步清理日志，不阻塞登录返回
          c.executionCtx.waitUntil(c.env.DB.prepare('DELETE FROM admin_logs WHERE id NOT IN (SELECT id FROM admin_logs ORDER BY id DESC LIMIT 50)').run());
...
app.get('/public/home', async (c) => {
  ...
  // 【核心优化】加入 CDN 缓存头
  // s-maxage: Cloudflare 边缘缓存时间
  // 如果有搜索条件，缓存短一点(10s)，如果是纯列表，缓存长一点(60s)
  const cacheTime = (q || tagId || catId) ? 10 : 60;
  c.header('Cache-Control', `public, max-age=${cacheTime}, s-maxage=${cacheTime}`);
...
app.get('/public/tags', async (c) => {
    ...
    const res = await c.env.DB.prepare(`SELECT DISTINCT t.* FROM tags t JOIN resource_tags rt ON t.id = rt.tag_id JOIN resources r ON rt.resource_id = r.id WHERE t.type = ? ORDER BY t.id DESC`).bind(type).all();
    // 标签列表变动不频繁，缓存 5 分钟
...
app.get('/user/info', async (c) => {
  ...
  const used = (await c.env.DB.prepare('SELECT COUNT(*) as c FROM unlocked_items WHERE user_id = ? AND date_str = ?').bind(user.id, today).first()).c;
  ...
  return c.json({ 
      user: { id: user.id, username: user.username, email: user.email, is_muted: user.is_muted, consecutive_days: user.consecutive_days || 0 }, 
...
app.get('/admin/dashboard', async (c) => {
    ...
    // 使用 Promise.all 并发查询，减少等待时间
    const [logs, userC, postC, msgC, unlockC] = await Promise.all([
        c.env.DB.prepare('SELECT * FROM admin_logs ORDER BY id DESC LIMIT 10').all(),
        c.env.DB.prepare('SELECT COUNT(*) as c FROM users').first(),
        c.env.DB.prepare('SELECT COUNT(*) as c FROM resources').first(),
        c.env.DB.prepare('SELECT COUNT(*) as c FROM messages').first(),
        c.env.DB.prepare('SELECT COUNT(*) as c FROM unlocked_items WHERE date_str = ?').bind(new Date().toISOString().split('T')[0]).first()
    ]);

    // 仪表盘数据不需要秒级实时，缓存 30 秒
...
app.get('/admin/resources', async (c) => {
    ...
    const page=parseInt(c.req.query('page')||'1'); const size=50; const off=(page-1)*size;
    
    // 移除 content_json，减少 IO
    let sql='SELECT DISTINCT r.id, r.title, r.category_id, r.custom_date, r.created_at FROM resources r LEFT JOIN resource_tags rt ON r.id=rt.resource_id LEFT JOIN tags t ON rt.tag_id=t.id';
    let p=[]; let k=[];
    ...
    sql+=' ORDER BY r.id DESC LIMIT ? OFFSET ?'; p.push(size,off);
