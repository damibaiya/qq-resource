...
app.post('/admin/resource/deduplicate', async (c) => {
    ...
    // 1. 找出有重复标题的 (去掉首尾空格后比较)
    // 注意：SQLite 的 TRIM() 可能需要根据环境支持，但通常 Cloudflare D1 支持
    const duplicates = await c.env.DB.prepare(`
        SELECT TRIM(title) as clean_title, COUNT(*) as cnt 
        FROM resources 
    ...
    for (const dup of duplicates.results) {
        // 找出该标题对应的所有 ID，按 ID 倒序排列 (最新的在前)
        const allPosts = await c.env.DB.prepare('SELECT id FROM resources WHERE TRIM(title) = ? ORDER BY id DESC').bind(dup.clean_title).all();
...
app.post('/admin/resource', async (c) => {
  ...
  try {
      if (id) {
        await c.env.DB.prepare('UPDATE resources SET title=?, category_id=?, content_json=?, custom_date=? WHERE id=?').bind(title.trim(), category_id, JSON.stringify(blocks), dateStr, id).run();
        await c.env.DB.prepare('DELETE FROM resource_tags WHERE resource_id = ?').bind(id).run();
      } else {
        // 新增前再次检查是否存在 (忽略空格)
        const exist = await c.env.DB.prepare('SELECT id FROM resources WHERE TRIM(title) = ?').bind(title.trim()).first();
        if (exist) return c.json({ error: '标题已存在 (重复)' }, 400);

        const res = await c.env.DB.prepare('INSERT INTO resources (title, category_id, content_json, custom_date) VALUES (?, ?, ?, ?) RETURNING id').bind(title.trim(), category_id, JSON.stringify(blocks), dateStr).first();
        resourceId = res.id;
      }
  } catch (e) {
      if (e.message.includes('UNIQUE')) return c.json({ error: '标题已存在 (数据库约束)' }, 400);
